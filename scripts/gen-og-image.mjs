 ...
 * never run in build/deploy. Brand fonts (Zilla Slab 700 + Oswald) are
 * base64-embedded so the render is fully offline. The signature score gauge
 * uses a dark-bg variant (cream needle/hub/number) so it reads on charcoal.
 *
 *   node scripts/gen-og-image.mjs        # -> og-image.png (1200x630 @2x)
 */
...
async function run() {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .right{flex:0 0 400px;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;border-left:1px solid rgba(251,238,216,.12);background:rgba(237,120,24,.05)}
  ...
  console.log(`wrote og-image.png (${W}x${H} @${SCALE}x)`);
}
run();
