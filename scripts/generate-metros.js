...
const METRO_LOCAL = {
  ...
  'dallas-fort-worth-tx': [
    'Dallas–Fort Worth gives a long smoke season bracketed by two hazards: spring storms and summer heat. The strongest windows are the shoulder months — March into May before the worst storms, then October and November once the heat breaks. Late spring brings the Metroplex’s signature severe weather, with frontal lines that drop wind, hail, and rain fast, so a long weekend cook in April or May needs a close read on the gust and storm forecast. July through September runs hot and often dry inland, which is workable but asks a lot of an exposed pit.',
  ...
  'houston-tx': [
    'Houston’s defining smoke variable is Gulf humidity. Dew points sit in the 70s for much of the year, which drives a high wet-bulb temperature and the long, stubborn stalls that define a Houston brisket cook. The most comfortable windows are late fall through early spring — November into March — when the air dries out and cools off. Summer is hot, sticky, and storm-prone, and the heart of hurricane season runs June through November, so any long-range weekend plan in late summer should carry a weather contingency.',
  ...
  'san-francisco-ca': [
    'San Francisco has the mildest, most stable smoking weather of any major US city, and the season essentially never closes. The marine climate keeps highs moderate year-round — rarely hot, rarely freezing — with low humidity and a dry stretch that runs spring through fall. The wettest months are December through March, but even then cookable weekends are common. The one constant is the afternoon wind and fog off the Pacific, which is the real variable a Bay Area pitmaster plans around rather than heat or storms.',
    'That marine wind drags an open offset’s pit temperature and can swing 15-to-20 mph as the fog rolls in, so a wind break earns its keep here. An inland yard a few miles back from the water — in the East Bay or South Bay — sees less of it and runs an offset comfortably most of the year. The low humidity means shorter stalls than the humid East, so a brisket moves a touch faster, but the dry, breezy air also pulls moisture, so favor butcher paper on the wrap. Pick a low-wind day off the score and the Bay Area calendar will give you a cook nearly any weekend.',
  ],
  'riverside-ca': [
